...
class ClamAvScanner extends AbstractService
{
    public function scan(string $localPath): array
    {
        ...
        $mode = strtolower((string)($this->app->options()->wrxtPfClamMode ?? 'unix'));
        $connectTimeout = max(1, min(15, (int)($this->app->options()->wrxtPfClamConnTimeout ?? 3)));
        $readTimeout = max(5, min(120, (int)($this->app->options()->wrxtPfClamReadTimeout ?? 30)));

        if ($mode === 'tcp')
        {
            $host = trim((string)($this->app->options()->wrxtPfClamHost ?? '127.0.0.1'));
            $port = max(1, min(65535, (int)($this->app->options()->wrxtPfClamPort ?? 3310)));
        ...
        {
            $socket = trim((string)($this->app->options()->wrxtPfClamSocket ?? '/run/clamav/clamd.ctl'));
            if ($socket === '' || $socket[0] !== '/')
            {
    ...
    public function ping(): bool
    {
        try
        {
            ...
            {
                $socketPath = trim((string)($this->app->options()->wrxtPfClamSocket ?? '/run/clamav/clamd.ctl'));
