...
class WorkerProcess extends AbstractService
{
    public function runImageWorker(string $input, string $displayOutput, string $thumbOutput): array
    {
        ...
        $memoryMb = max(64, min(1024, (int)$this->app->options()->wrxtPfWorkerMemMb));
        $timeout = max(5, min(120, (int)$this->app->options()->wrxtPfWorkerTimeout));
        $displayMax = max(512, min(8192, (int)$this->app->options()->wrxtPfDisplayMaxDim));
        $thumbMax = max(128, min(2048, (int)$this->app->options()->wrxtPfThumbMaxDim));
        $quality = max(50, min(95, (int)$this->app->options()->wrxtPfWebpQuality));
    ...
    {
        $config = [
            'vertices' => max(1000, (int)$this->app->options()->wrxtPfModelVertices),
            'triangles' => max(1000, (int)$this->app->options()->wrxtPfModelTriangles),
            'meshes' => max(1, (int)$this->app->options()->wrxtPfModelMeshes),
            'primitives' => max(1, (int)$this->app->options()->wrxtPfModelPrimitives),
            'nodes' => max(1, (int)$this->app->options()->wrxtPfModelNodes),
            'materials' => max(1, (int)$this->app->options()->wrxtPfModelMaterials),
            'textures' => max(0, (int)$this->app->options()->wrxtPfModelTextures),
            'animations' => max(0, (int)$this->app->options()->wrxtPfModelAnimations),
            'skins' => max(0, (int)$this->app->options()->wrxtPfModelSkins),
            'joints' => max(0, min(48, (int)$this->app->options()->wrxtPfModelJoints)),
            'accessors' => max(1, (int)$this->app->options()->wrxtPfModelAccessors),
            'bufferViews' => max(1, (int)$this->app->options()->wrxtPfModelBufViews),
            'accessorElements' => max(1, (int)$this->app->options()->wrxtPfModelAccElems),
            'textureDimension' => max(128, (int)$this->app->options()->wrxtPfModelTexDim),
            'textureBytes' => max(1024, (int)$this->app->options()->wrxtPfModelTexBytes),
            'animationKeyframes' => max(1, (int)$this->app->options()->wrxtPfModelAnimKeys),
            'depth' => max(8, min(256, (int)$this->app->options()->wrxtPfModelNodeDepth))
        ];
    ...
    {
        $configured = trim((string)$this->app->options()->wrxtPfWorkerPhp);
