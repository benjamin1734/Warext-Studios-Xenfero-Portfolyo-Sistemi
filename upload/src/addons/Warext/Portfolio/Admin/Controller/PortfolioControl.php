...
class PortfolioControl extends Portfolio
{
    public function actionQuarantineRetry()
    {
        ...
        try
        {
            $result = $this->runFileNow($file);
        }
        catch (\Throwable $e)
        {
            \XF::logException($e, false, 'Warext Portfolio manual processing: ');
            return $this->error('Dosya işlenirken hata oluştu: ' . $this->safeQueueReason($e->getMessage()));
        }
        ...
        return $this->redirectAfterQueueRun($file, $result);
    }
    ...
    public function actionQuarantineContinue()
    {
        ...
        try
        {
            $result = (string)$this->service('Warext\Portfolio:ProcessingPipeline')->process($file);
        }
        catch (\Throwable $e)
        {
            \XF::logException($e, false, 'Warext Portfolio ClamAV bypass processing: ');
            return $this->error('ClamAV adımı geçildi fakat dosya işlenemedi: ' . $this->safeQueueReason($e->getMessage()));
        }
    ...
    private function runFileNow($file): string
    {
        ...
        if (in_array($state, ['quarantine', 'validating', 'scanning'], true))
        {
            $file->next_scan_date = 0;
            $file->save();
            return (string)$this->service('Warext\Portfolio:SecurityPipeline')->process($file);
        }
    ...
    private function redirectAfterQueueRun($file, string $result)
    {
        ...
        {
            'scan_pending' => 'Dosya doğrulandı fakat ClamAV erişilemiyor. Güvenli koşullarda “ClamAV olmadan devam” seçeneğini kullanabilirsiniz.',
            'processing_pending' => 'Dosya işlenemedi. Güncel teknik neden: ' . ($reason ?: 'bilinmiyor'),
            ...
            default => 'İşlem tamamlandı. Güncel durum: ' . $result
        };

        return $this->redirect($this->buildLink('wrxt-portfolyo/quarantine'), $message);
    }

    private function safeQueueReason(string $reason): string
    {
        $reason = trim($reason);
        if ($reason === '')
        {
