...
class UserPortfolio extends AbstractController
{
    public function actionIndex(ParameterBag $params)
    {
        ...
        $perPage = 24;
        $finder = $this->repository('Warext\Portfolio:Portfolio')->findPublishedForUser($user->user_id);
        $total = $finder->total();
        $items = $finder->limitByPage($page, $perPage)->fetch();
        $community = $this->service('Warext\Portfolio:Community');
        $stats = $community->followStats((int)$user->user_id);

        return $this->view('Warext\Portfolio:User\Portfolio', 'wrxt_portfolio_user', [
            'user' => $user,
            'items' => $items,
            'page' => $page,
            'perPage' => $perPage,
            'total' => $total,
            'isFollowing' => $community->isFollowing((int)$user->user_id),
            'followStats' => $stats
        ]);
    }
    ...
    public function actionFollow(ParameterBag $params)
    {
        ...
        try
        {
            $this->service('Warext\Portfolio:Community')->toggleFollow($user);
        }
