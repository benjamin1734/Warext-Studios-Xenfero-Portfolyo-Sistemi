...
class ModerationManager extends AbstractService
{
    ...
    {
        $rows = $this->db()->fetchAll("SELECT file_id, state, processing_status, processed_mime FROM xf_wrxt_portfolio_file WHERE portfolio_id = ? AND state <> 'deleted' FOR UPDATE", (int)$portfolio->portfolio_id);
        if (!$rows)
        {
        ...
        foreach ($rows as $row)
        {
            if (!in_array((string)$row['state'], ['security_passed', 'moderation', 'published'], true) || (string)$row['processing_status'] !== 'passed')
            {
                throw new \RuntimeException('wrxt_portfolio_file_not_security_passed');
            }
            $file = $this->em()->find('Warext\Portfolio:PortfolioFile', (int)$row['file_id']);
            if (!$file)
            {
                throw new \RuntimeException('wrxt_portfolio_file_missing');
            }
            $expectedMime = (string)$file->file_role === 'model' ? 'model/gltf-binary' : 'image/webp';
            if ((string)$row['processed_mime'] !== $expectedMime || !$blobManager->isFileStorageSafe($file, false))
            {
                throw new \RuntimeException('wrxt_portfolio_file_not_security_passed');
            }
            if ((string)$file->file_role !== 'model' && !$blobManager->isFileStorageSafe($file, true))
            {
                throw new \RuntimeException('wrxt_portfolio_thumbnail_not_security_passed');
            }
        }
    }
